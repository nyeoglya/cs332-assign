...
object Main {
  def main(args: Array[String]) = {
    ...
    val terminationRequested: Future[String] = {
      val timeoutFuture: Future[String] = Future.delay(20.seconds).map(_ => "Timeout elapsed")
      val userInputFuture: Future[String] = Future.userInput("Press ENTER to terminate: ")
      Future.firstCompletedOf(List(timeoutFuture, userInputFuture))
    }
    ...
    terminationRequested onComplete {
      ...
      case Failure(ex) =>
        println(s"An error occurred while waiting for termination: ${ex.getMessage}")
    }
  }
}
